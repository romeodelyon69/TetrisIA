...
class DQNAgent:
    ...
    def best_state(self, states, render):
        ...
        if random.random() <= self.epsilon:
            if(render):
                print("i played randomly to find out new strategies !")
                '''for state in states:
                    if state[0]>1:
                        return state'''
            ...
            for state in states:
                ...
                if not max_value or value > max_value:
                    ...
                    best_state = state
            if(render):
                state_clean_line = np.array([1, 0, 0, 0, 0])
                print("la valeur de l'état [1, 0, 0, 0, 0] est la suivante : ", self.predict_value(np.reshape(state_clean_line, [1, self.state_size])))
                print("la valeur du meilleure état est : ", max_value, " pour l'état : ", best_state)
        return best_state


    def train(self, batch_size=32, epochs=3, render = False):
        '''Trains the agent'''
        ...
        if n >= self.replay_start_size and n >= batch_size:
            # S'assurer que k ne dépasse pas la taille de l'array

            # Choisir un indice de départ aléatoire
            indice_depart = random.randint(0, len(self.memory) - batch_size)

            # Extraire les k éléments consécutifs
            batch = list(islice(self.memory, indice_depart, indice_depart + batch_size))
            #batch = random.sample(self.memory, batch_size)   IL FAUT IMPERATIVEMENT QUE LES ELEMENTS SOIENT CONSECUTIFS MDRRR ON FAIT UN Q LEARNING C4EST FOU CE QU4ON TROUVE SUR INTERNET QUAND MEME
            ...
            for i, (state, _, reward, done) in enumerate(batch):
                ...
                if(render):
                    print("state : ", state, " et la valeur associée : ", new_q)
